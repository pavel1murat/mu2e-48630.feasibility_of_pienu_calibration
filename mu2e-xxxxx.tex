...
\begin{titlepage}
 ...
 \end{flushright}
  ...
  \begin{abstract}
    ...
    Success of the \piplusenu\ peak reconstruction critically depends on
    suppression of the muon decays in flight (DIF). We establish selections
    which drastically reduce the DIF background. 

    The simulated DIF statistics corresponding to about $8 \times 10^{12}$ 
    protons on target (POT) points to that a favorable signal to background (S/B) ratio
    can be achieved.
    However, drawing a conclusion about the feasibility of the \piplusenu\ calibration
    requires increasing the simulated DIF statistics by an order of magnitude,
    to the level above $10^{14}$ POT.
  \end{abstract}
