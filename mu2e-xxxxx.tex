 ...
 \end{flushright}
  ...
  \begin{center}
    {\Large \bf On the feasibility of using $\pi^+ \to e^+ \nu$ decays \\
      for the Mu2e momentum scale calibration.
  ...
  \begin{abstract}
    ...
    \vspace{0.2in}
    This note discusses the feasibility of using the \piplusenu\ decays
    of positive pions stopped in the stopping target (ST) for the Mu2e
    momentum scale calibration. He calibration requires reconstructing
    the \piplusenu\ peak and critically depends on  suppression
    of the $\mu^+ \to e^+ \nu_e \bar{\nu_{\mu}} $ decays in flight (DIF). 

    The analysis based on the simulated DIF statistics of $\sim 8 \times 10^{12}$ 
    protons on target (POT) establishes event selection which drastically reduces
    the DIF background and points to that a favorable signal to background (S/B)
    ratio can be achieved. However, drawing a conclusion about the feasibility
    of the \piplusenu\ calibration requires increasing the simulated DIF statistics
