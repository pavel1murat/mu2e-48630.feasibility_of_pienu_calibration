...
\begin{titlepage}
 ...
 \end{flushright}
  ...
  \begin{abstract}
    \vspace{0.2in}
    A possibility of using \piplusenu\ decays of positive pions stopped
    in the stopping target (ST) for the Mu2e momentum scale calibration
    has been long discussed within the collaboration.

    The calibration relies on the reconstruction of the \piplusenu\ peak,
    and its success critically depends on suppression of the $\mu^+ \to e^+ \nu_e \bar{\nu_{\mu}}$
    decays in flight (DIF).

    Together with the measurement of the edge of the Michel spectrum at B = 0.5 T,
    reconstruction of the 70 MeV/c positron peak from \piplusenu\ at B = 0.7 T
    would allow to calibrate the Mu2e momentum scale at B = 1.0 T.
    
    The analysis presented in this note introduces the event selection
    procedure which significantly reduces the DIF background and indicates 
    that a signal to background (S/B) ratio, sufficient for the successful
    \piplusenu\ peak reconstruction, can be achieved.

    The analysis is based on the simulated DIF statistics equivalent to
    $\sim 8 \times 10^{12}$ protons on target (POT).
    However, drawing a conclusion about the feasibility of reconstructing 
    the \piplusenu\ channel requires increasing the simulated DIF statistics
    by another order of magnitude, to the level above $10^{14}$ POT.
  \end{abstract}
