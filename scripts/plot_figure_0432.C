//-----------------------------------------------------------------------------
// tight ID, 2mm DEG: momentum for STT, DEG and DIF
//-----------------------------------------------------------------------------
plot_data_t* plot_figure_0432(int Figure, int Print) {
...
// 1.23e-4 : BR(pi --> e nu)
//------------------------------------------------------------------------------
  p.hd[0]              = hist_data_t(catalog,book,bpip4b0s51,ana_job,ana_module,hist_name);
  ...
  p.hd[1]              = hist_data_t(catalog,book,bpip4b0s54,ana_job,ana_module,hist_name);
  p.hd[1].fRebin       = 5;
  p.hd[1].fLabel       = "Degrader";
  p.hd[1].fMarkerColor = kBlue;
  p.hd[1].fMarkerStyle = 21;
  p.hd[1].fMarkerSize  = 0.8;
  p.hd[1].fLumiSF      = gPipenu->GetChannel("bpip4b0s54r0100")->NormSF();
//-----------------------------------------------------------------------------
// no need to weigh the DIF histogram with the pion survival prob
//-----------------------------------------------------------------------------
  p.hd[2]              = hist_data_t(catalog,book,bmup4b0s54,ana_job,ana_module,"trk_128/p_2");
  p.hd[2].fRebin       = 5;
  p.hd[2].fLabel       = "DIF";
