//-----------------------------------------------------------------------------
// 5 mm DEG: trk_178/p_2 vs trk_158/p_2 for DIF (loose vs  tight ID)
//-----------------------------------------------------------------------------
plot_data_t* plot_figure_0533(int Figure, int Print) {
...
// 1.23e-4 : BR(pi --> e nu)
//------------------------------------------------------------------------------
  p.hd[0]              = hist_data_t(catalog,book,bmup5b0s54,ana_job,ana_module,"trk_106/p_2");
  p.hd[0].fRebin       = 5;
  p.hd[0].fLabel       = "DIF, loose cuts";
  p.hd[0].fLineColor   = kGreen+3;
  p.hd[0].fMarkerColor = kGreen+3;
  p.hd[0].fMarkerStyle = 24;
  p.hd[0].fLumiSF      = gPipenu->GetChannel("bmup5b0s54r0100")->NormSF();
//-----------------------------------------------------------------------------
// no need to weigh the DIF histogram with the pion survival prob
//-----------------------------------------------------------------------------
  p.hd[1]              = hist_data_t(catalog,book,bmup5b0s54,ana_job,ana_module,"trk_126/p_2");
  p.hd[1].fRebin       = 5;
  ...
  p.hd[1].fLumiSF      = gPipenu->GetChannel("bmup5b0s54r0100")->NormSF();
