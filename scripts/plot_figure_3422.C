...
plot_data_t* plot_figure_3422(int Figure, int Print) {
...
// 1.23e-4 : BR(pi --> e nu)
//------------------------------------------------------------------------------
  p.hd[0]              = hist_data_t(catalog,"pipenu",bmup5b0s24,ana_job,"murat_SpmcAna","simp_222/stime");
  p.hd[0].fRebin       = 1;
  ...
  p.hd[1]              = hist_data_t(catalog,"pipenu",bmup5b0s25,ana_job,"murat_SpmcAna","simp_222/stime");
  p.hd[1].fRebin       = 1;
  ...
  p.fLabel             = "#mu^{+} decay time; 5 mm Ti degrader; p(e^{+} > 55 MeV/c";
  p.fYLogScale         = 1;
